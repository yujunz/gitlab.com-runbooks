...
to do so you will need to issue the command `bundle exec rake "edit_role[gitlab-fe-lb]"` from the
...
`knife ssh -p 2222 -C 1 -a ipaddress role:gitlab-fe-lb  'sudo chef-client'`
...
```
knife ssh -p 2222 -a ipaddress -C 2 role:gitlab-fe-lb " 
echo 'disable server https_git/git01.fe.gitlab.com' | sudo socat stdio /run/haproxy/admin.sock"
```
...
```
knife ssh -p 2222 -a ipaddress -C 2 role:gitlab-fe-lb "
echo 'enable server https_git/git01.fe.gitlab.com' | sudo socat stdio /run/haproxy/admin.sock"
```
