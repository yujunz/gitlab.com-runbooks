...
local legacySort = (import 'legacy_sort.libsonnet').legacySort;
  ...
  ['key-metrics-%s.yml' % [service.type]]:
    outputPromYaml(
      [{
        ...
        interval: '1m',
        rules: legacySort(configMap.prometheus.componentMetrics.generateRecordingRulesForService(service)),
      }]
      +
      (
        if ({ nodeLevelMonitoring: false } + service).nodeLevelMonitoring then
          [{
            ...
            interval: '1m',
            rules: legacySort(configMap.prometheus.nodeMetrics.generateRecordingRulesForService(service)),
          }]
        else
          []
      )
