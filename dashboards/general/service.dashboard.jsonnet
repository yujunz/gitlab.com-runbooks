...
.addPanel(
  ...
  .addTarget(
    prometheus.target('
      sum(
        gitlab_service_ops:rate:avg_over_time_1w{component="", service="", environment="$environment", type="$type"} +
        $sigma * gitlab_service_ops:rate:stddev_over_time_1w{component="", environment="$environment", type="$type"}
      )
      ',
      interval="1m",
      legendFormat='upper normal',
...
.addPanel(
  generalGraphPanel(
    "Service Operation Rates - per Second (Iteration 2)",
    description="The operation rate is the sum total of all requests being handle for all components within this service. Note that a single user request can lead to requests to multiple components. Higher is busier."
  )
  .addTarget( // Primary metric
    prometheus.target('
...
.addPanel(
  generalGraphPanel(
    "Service Error Rates - per Minute",
  ...
  .addTarget( // Primary metric
    prometheus.target('
      sum(
        avg_over_time(
          gitlab_service_errors:rate{component="", service="", environment="$environment", type="$type"}[$__interval]
        )
      ) by (type) * 60
      ',
      interval="1m",
  ...
  .addTarget( // Last week
    prometheus.target('
      sum(
        avg_over_time(
          gitlab_service_errors:rate{component="", service="", environment="$environment", type="$type"}[$__interval] offset 1w
        )
      ) by (type) * 60
      ',
      interval="1m",
  ...
  .addTarget(
    prometheus.target('
      sum(
        (
          gitlab_service_errors:rate:avg_over_time_1w{component="", service="", environment="$environment", type="$type"} +
          $sigma * gitlab_service_errors:rate:stddev_over_time_1w{component="", service="", environment="$environment", type="$type"}
        ) * 60
      )
      ',
      interval="1m",
  ...
  .addTarget(
    prometheus.target('
      sum(
        clamp_min(
          gitlab_service_errors:rate:avg_over_time_1w{component="", service="", environment="$environment", type="$type"} -
          $sigma * gitlab_service_errors:rate:stddev_over_time_1w{environment="$environment", type="$type"},
          0
        ) * 60
      )
      ',
